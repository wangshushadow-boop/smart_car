"""统一全模态 Agent ROS 2 Service Server 入口。

这是 `llm_agent` 在 WSL 上的唯一进程入口，启动顺序：
1. 单实例锁：防止同一台主机上重复启动 Service 抢占 `/car/agent/run`。
2. 配置加载：根据 `agent.yaml` 与环境变量构建 `AgentConfig`。
3. Runtime 装配：构造 Reactor、DialogueLoop、TaskManager 与 SkillRunner。
4. Gateway 装配：增加请求幂等和同 Session 串行控制。
5. Service Server 启动：在 ROS 2 多线程 Executor 上提供短对话接口。
6. 信号处理：`Ctrl+C` 或 ROS 外部关闭信号触发优雅退出。
"""
...
from llm_agent.runtime.runtime import create_runtime
...
def _configure_model_output_logger() -> None:
    ...
    `Reactor` 在解析每轮结构化决策前写入 `llm_agent.model_output` logger；
...
def _single_instance_lock() -> Iterator[TextIO]:
    ...
    try:
        try:
            ...
            raise RuntimeError(
                "Agent Server 已在运行，拒绝启动重复的 ROS Service Server"
            ) from error
        yield lock_file
    finally:
        lock_file.close()
...
def main() -> None:
    """进程入口：装配 Runtime 并运行 Service Server 直到退出。"""
    try:
        with _single_instance_lock():
            ...
            # 多线程 Executor：对话 Service、Tool Action 和音频 Service 可并发。
