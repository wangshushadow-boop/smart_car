...
| Service | 类型 | 说明 |
| --- | --- | --- |
| `/car/agent/run` | `small_car_interfaces/srv/RunAgent` | 上传一轮多模态输入，返回对话或任务受理结果 |
| `/car/audio/enqueue` | `small_car_interfaces/srv/PlayAudio` | Agent Server 提交最终 WAV；树莓派确认接收后后台播放，不等待播放结束 |

长时间、需要取消和反馈的机器人 Tool 仍使用 `/car/agent/tool_execute`
`small_car_interfaces/action/ExecuteRobotTool`。

`agent_client/car_agent_client` 在树莓派侧把 VAD 完成的 WAV 和最近 JPEG 组成 Service 请求，并处理返回的
文字与状态。最终播报由独立 Service 主动下发；Web Debug 也只使用统一 Service，因此调试代码不进入
Agent 或树莓派客户端。
