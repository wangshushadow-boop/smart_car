...
RUNTIME_VENV="${MINICPM_RUNTIME_VENV:-${SCRIPT_DIR}/../py_env/venvs/minicpm}"
VLLM_BIN="${RUNTIME_VENV}/bin/vllm"
...
if [[ ! -x "${VLLM_BIN}" || ! -f "${MODEL_DIR}/config.json" || ! -f "${DEPLOY_CONFIG}" || ! -x "${CUDA_NVCC}" ]]; then
  echo "错误：复用环境中缺少 vLLM-Omni，或模型配置不完整。"
  echo "运行环境：${RUNTIME_VENV}"
  echo "模型目录：${MODEL_DIR}"
  exit 1
fi
if pgrep -f "${RUNTIME_VENV}/bin/vllm serve" >/dev/null; then
